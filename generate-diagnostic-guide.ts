...
async function generateDiagnosticGuide() {
  const reportPath = path.join(
    process.cwd(),
    "AUTH_USERS_DIAGNOSTIC_REPORT.md"
  );

  const report = `# 🔍 Rapport Diagnostic: auth.users - Triggers et RLS
...
`;
  ...
  console.log("\n" + "═".repeat(90));
  console.log("📊 RAPPORT DE DIAGNOSTIC CRÉÉ");
  console.log("═".repeat(90));
  console.log(
    `\n✅ Fichier généré: AUTH_USERS_DIAGNOSTIC_REPORT.md\n`
  );
  console.log(report);
  console.log(
    "\n💾 Le rapport complet a été sauvegardé dans: AUTH_USERS_DIAGNOSTIC_REPORT.md\n"
  );
}

generateDiagnosticGuide().catch(console.error);
